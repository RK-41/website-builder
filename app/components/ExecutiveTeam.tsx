    ...
    <section className="bg-white dark:bg-gray-900 py-10">
      <Container maxWidth="lg">
        <Typography
          variant="h4"
          align="center"
          className="text-gray-800 dark:text-white capitalize font-semibold"
        >
          {title}
        </Typography>
        {description && (
          <Typography
            variant="body1"
            align="center"
            className="text-gray-500 dark:text-gray-300 max-w-2xl mx-auto my-6"
          >
            {description}
          </Typography>
        )}
          ...
          {members.map((member, index) => (
            <Grid key={index} size={{ xs: 12, md: 6 }}>
              <Card className="group border border-gray-200 dark:border-gray-700 hover:border-transparent hover:bg-blue-600 transition-colors duration-300 rounded-xl px-4 py-6 cursor-pointer">
                <CardHeader
                  avatar={
                    <Avatar src={member.image} sx={{ width: 64, height: 64 }} />
                  }
                  title={
                    <Typography
                      variant="h6"
                      className="text-gray-700 dark:text-white group-hover:text-white capitalize"
                    >
                      {member.name}
                    </Typography>
                  }
                  ...
                  <div className="flex mt-4">
                    {member.linkedin && (
                      <Link
                        ...
                        className="mx-2 text-gray-600 dark:text-gray-300 group-hover:text-white"
                      >
                        <LinkedInIcon />
                      </Link>
                    )}
