...
export function Client({ path, data }: { path: string; data: Partial<Data> }) {
  const localDatabase =
    typeof window !== 'undefined'
      ? localStorage.getItem('puck-database')
      : null;

  const [database, setDatabase] = useState<Database>(
    localDatabase ? JSON.parse(localDatabase) : initialDatabase
  );
  ...
  const handlePublish = async (
    newData: any,
    newPath: string = currentPage.path
  ) => {
    ...
    localStorage.setItem('puck-database', JSON.stringify(data.data));
